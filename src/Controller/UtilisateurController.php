...
class UtilisateurController extends AbstractController
{
        ...
        public function index(UtilisateurRepository $utilisateurRepository, ProjetRepository $projetRepository, Session $session): Response
        {
                ...
                if (!$utilisateur) {
                        ...
                        return $this->render('navigation/admin.html.twig',  [
                                'projets' => $listEnCours, 'projetsTerminés' => $listTerminés, 'projetsDébut' => $listDebut, 'users' => $listUsers, 'docs' => $listDocs, 'equipes' => $equipes
                        ]);
                }
        ...
        public function affectUser(Request $request, Session $session, Equipe $equipe2): Response
        {
                ...
                if ($form->isSubmitted() && $form->isValid()) {
                        ...
                        $equipe->setProjet($pp);
                        //dd($equipe2->getNom());
                        $equipe->setNom($equipe2->getNom());
                        $equipe->setGerant($equipe2->getGerant());
        ...
        /**
         * @Route("/{id}/assignUser",name="assign_user")
         * @param Request $request
         * @return Response
         */

        public function assignUser(Request $request, Session $session, Equipe $eq): Response
        {
                //dd($session->get('docum')->getId());
                //        $doc = $this->getDoctrine()->getRepository(Document::class);
                //        $pr=$doc->findOneBy(["id"=>$id]);
        ...
        public function deleteUserFromEq(Request $request, Session $session, Equipe $eq): Response
        {
                ...
                if ($form->isSubmitted() && $form->isValid()) {
                        ...
                        return $this->render('utilisateur/show.html.twig', [
                                'eq' => $eqq, 'users' => $list, 'u' => $utilisateur
                        ]);
                }
        ...
        public function showUser(ProjetRepository $projetRepository, Equipe $equipe): Response
        {
                ...
                //dd($equipe->getMembre());
                return $this->render('utilisateur/show.html.twig',  [
                        'users' =>  $list, 'eq' => $equipe, 'u' => $utilisateur
                ]);
        }
     ...
     $listSousRep->addSousRepertoire($pp);*/
        ...
        /**
         * @Route("/repertoire/{id}/newSousRepertoire",name="sous_repertoire_new")
         * @param Request $request
         * @return Response
         */

        public function newSousRepertoire(repertoire $repertoire, Request $request, Session $session): Response
        {
                $sousRepertoire = new Repertoire();
        
                $form = $this->createForm(RepertoireType::class, $sousRepertoire);
                $form->handleRequest($request);
        
              
                if ($form->isSubmitted() && $form->isValid()) {
                
                        $sousRepertoire->setEquipe($repertoire->getEquipe());
                      
                        $repertoire->addSousRepertoire($sousRepertoire);
                 
                        $em = $this->getDoctrine()->getManager();

                        $em->persist($sousRepertoire);
